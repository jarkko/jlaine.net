...
\# **Optimistic locking.** Just read about this one in the mailing list. Add a field called `lock_version` (integer) into your table and ActiveRecord will use optimistic locking for that table from now on, as long as you put a hidden field with that version to your edit form.\
