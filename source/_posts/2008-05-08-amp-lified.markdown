...
```
<code class="ruby">yell = lambda { puts "AAARGH!!!" }
whisper = Proc.new do
 puts "whee"
end
yell.call
...
```
<code class="ruby">>> arr = %w(apple orange kiwi)
=> ["apple", "orange", "kiwi"]
>> arr.each {|i| puts i[0,2]}
ap
or
ki
=> ["apple", "orange", "kiwi"]</code>
```
...
```
<code class="ruby">>> put_two = lambda {|i| puts i[0,2]}
=> #<Proc:0x00329060@(irb):41>
>> arr.each(put_two)</code>
```
...
```
<code class="ruby">>> arr.each(put_two)
...
```
<code class="ruby">>> arr.map(&:length)</code>
```

It is effectively the same as

```
<code class="ruby">>> arr.map {|i| i.length }
=> [5, 6, 4]</code>
```
...
```
<code class="ruby">>> arr.map(&:length)
...
```
<code class="ruby">>> "Nice array: " + arr
...
>> "Nice array: " + arr.to_s
=> "Nice array: appleorangekiwi"</code>
```
...
~~Here we make `to_proc` a lambda function that will use the `send` method to call the method with the same name as the `Symbol` object in question (self) for the element that's passed to it.~~ *[]{.examples .use .just .let's .so .confusing .too .got .That}*. So

```
<code class="ruby">arr.send(:length)</code>
```

is the same as

```
<code class="ruby">arr.length</code>
```

And thus

```
<code class="ruby">:length.to_proc</code>
```

would become

```
<code class="ruby">lambda {|i| i.send(:length)}</code>
```

Now, let's see how our new method performs:

```
<code class="ruby">>> arr.map(&:reverse)
=> ["elppa", "egnaro", "iwik"]</code>
```
