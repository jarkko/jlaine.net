...
Many otherwise great web sites/applications, like [Basecamp](http://www.basecamphq.com) and [IconBuffet](https://www.iconbuffet.com/), seem to share one very annoying shortcoming: [their](http://odesign.clientsection.com/login) [forms](https://www.iconbuffet.com/account/login) don't use [*label*](http://www.w3schools.com/tags/tag_label.asp) elements.

Even if you think [accessibility is for documents](http://37signals.com/svn/archives2/fly_on_the_wall_week_of_march_10.php) (which it isn't), please use *labels* in your forms. They are not just for blind people.
...
`<input type="radio" name="sex" id="male" />`{=html} MALE`<br/>`{=html}\
`<input type="radio" name="sex" id="female" />`{=html} `<label for="female">`{=html}FEMALE (with label)`</label>`{=html}`<br/>`{=html}
