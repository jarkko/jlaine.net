...
`<filter:code>`{=html}sudo gem install mysql --- ---with-mysql-dir=/usr/local/mysql\
`</filter:code>`{=html}

Unfortunately, this resulted in errors:

`<filter:code>`{=html}Probutanol:\~ jarkko\$ sudo gem install mysql --- ---with-mysql-dir=/usr/local/mysql/\
...
`<filter:code>`{=html}#define ulong unsigned long\
`</filter:code>`{=html}
...
`<filter:code>`{=html}cd /opt/local/lib/ruby/gems/1.8/gems/mysql-2.7\
...
from /opt/local/lib/ruby/vendor_ruby/1.8/rubygems/custom_require.rb:21:in \`require\_\_'\
