...
- *checkcommand* is the command daedalus executes in order to find out if your target is working or not. In local monitoring it's most often "/bin/ps axww". Be sure to include the two w's in there, you'll be sorry one day if you don't[^1].
- *checkregex*: this is the regular expression daedalus will try to find from the output of *checkcommand*. So `/lighttpd/` means that we're looking for...er... lighttpd. Brilliant! If you're not yet familiar with regular expressions, here's a [resource](http://www.regular-expressions.info/) with a [tutorial](http://www.regular-expressions.info/) that might help you.
...
*[]{.environment. .any .in .it .emulate .easily .can .you .but .Rails .for .is .advice .This}*
