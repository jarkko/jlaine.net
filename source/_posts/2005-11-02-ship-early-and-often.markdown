...
> Does ship-early-and-often really work for a huge company doing massive PR pushes that's going to get millions of people checking out their early release?`<br />`{=html}`<br />`{=html}I don't think it does.
...
This is a clear misjudgement of agile or `<acronym title="Extreme Programming">`{=html}XP`</acronym>`{=html} methods. Like the pals at [37signals](http://37signals.com) say, "create half a product, not a half-assed product". So the idea is not to release a half-baked product early. It is to first create a product that does *only* the thing that's most crucial for it, and does it *well*.
